...
The workbench is organized around four views:

1. **Benchmark** for strict corpus-backed tokenizer comparisons
2. **Catalog** for deployable model metadata and tokenizer mappings
3. **Scenario Lab** for cost, scale, and context tradeoff analysis
4. **Audit** for formulas, definitions, sources, provenance, and caveats

### Suggested workflow

- Start in **Benchmark**
- Move to **Catalog**
- Test assumptions in **Scenario Lab**
- Verify numbers in **Audit**

### Data policy

- Strict verified values appear by default
- Proxy mappings are hidden unless explicitly enabled
- Scenario outputs are derived from your assumptions plus benchmark and catalog inputs
- Latency and throughput only appear when surfaced metadata exists
