...
class TestGetTokenizer:
    ...
    def test_returns_tokenizer_for_gpt2(self):
        ...
        import workbench.tokenizer as tok_module
        from workbench.tokenizer import get_tokenizer

        mock_tok = MagicMock()
        with patch.dict(tok_module._tokenizer_cache, {}, clear=True):
            with patch("workbench.tokenizer.AutoTokenizer.from_pretrained", return_value=mock_tok):
    ...
    def test_calls_from_pretrained_with_correct_repo_id_for_gpt2(self):
        ...
        with patch.dict(tok_module._tokenizer_cache, {}, clear=True):
            with patch("workbench.tokenizer._local_snapshot_path", return_value=None):
                with patch("workbench.tokenizer.AutoTokenizer.from_pretrained", return_value=MagicMock()) as mock_fp:
                    get_tokenizer("gpt2")
    ...
    def test_prefers_local_snapshot_path_for_hf_tokenizers(self):
        ...
        mock_tok = MagicMock()
        with patch.dict(tok_module._tokenizer_cache, {}, clear=True):
            with patch("workbench.tokenizer._local_snapshot_path", return_value="/tmp/qwen-local"):
                with patch("workbench.tokenizer.AutoTokenizer.from_pretrained", return_value=mock_tok) as mock_fp:
    ...
    def test_load_auto_tokenizer_falls_back_when_fix_flag_unsupported(self):
        ...
        with patch("workbench.tokenizer.AutoTokenizer.from_pretrained", side_effect=fake_from_pretrained):
    ...
    def test_local_snapshot_path_prefers_manifest_entry(self):
        ...
        with patch.object(tok_module, "_snapshot_manifest_cache", {"example/repo": "/tmp/example-snapshot"}):
            with patch("workbench.tokenizer.Path.exists", return_value=True):
                with patch("workbench.tokenizer._get_snapshot_download") as mock_snapshot_download:
    ...
    def test_list_tokenizer_snapshot_status_reports_ready_and_missing(self):
        ...
        with patch("workbench.tokenizer._local_snapshot_path", side_effect=fake_local_snapshot_path):
    ...
    def test_supported_tokenizers_come_from_shared_family_registry(self):
        ...
        from workbench.tokenizer import SUPPORTED_TOKENIZERS
        from workbench.tokenizer_registry import supported_tokenizers_map
    ...
    def test_evicts_oldest_cached_tokenizer_when_cache_limit_is_exceeded(self):
        ...
        with patch.dict(tok_module._tokenizer_cache, {}, clear=True):
            with patch.object(tok_module, "_TOKENIZER_CACHE_MAX_SIZE", 2):
                with patch("workbench.tokenizer.AutoTokenizer.from_pretrained", side_effect=fake_tokenizers):
                    first = tok_module.get_tokenizer("gpt2")
...
class TestGetTokenizerErrorHandling:
    ...
    def test_from_pretrained_failure_raises_clear_message(self):
        ...
        with patch.dict(tok_module._tokenizer_cache, {}, clear=True):
            with patch("workbench.tokenizer.AutoTokenizer.from_pretrained",
    ...
    def test_raises_without_network_fallback_when_local_cache_misses(self):
        ...
        with patch.dict(tok_module._tokenizer_cache, {}, clear=True):
            with patch("workbench.tokenizer._local_snapshot_path", return_value=None):
                with patch(
                    "workbench.tokenizer.AutoTokenizer.from_pretrained",
...
class TestTiktokenAdapter:
    ...
    def test_adapter_encode_returns_list_of_ints(self):
        ...
        from workbench.tokenizer import TiktokenAdapter
    ...
    def test_get_tokenizer_loads_tiktoken_for_o200k(self):
        ...
        import workbench.tokenizer as tok_module
        from workbench.tokenizer import TiktokenAdapter, get_tokenizer
    ...
    def test_tokenize_text_works_with_tiktoken_adapter(self):
        ...
        from workbench.tokenizer import TiktokenAdapter, tokenize_text
    ...
    def test_fragmentation_ratio_works_with_tiktoken_adapter(self):
        ...
        from workbench.tokenizer import TiktokenAdapter, fragmentation_ratio
...
class TestTokenizeText:
    ...
    def test_returns_list(self):
        ...
        from workbench.tokenizer import tokenize_text
...
class TestFragmentationRatio:
    ...
    def test_returns_dict(self):
        ...
        from workbench.tokenizer import fragmentation_ratio
...
class TestIsNonSpaceDelimited:
    ...
    def test_pure_cjk_unified_returns_true(self):
        ...
        from workbench.tokenizer import _is_non_space_delimited
...
class TestFlagOovWords:
    ...
    def test_returns_set(self):
        ...
        from workbench.tokenizer import flag_oov_words
...
class TestDetectLanguage:
    ...
    def test_returns_string(self):
        ...
        from workbench.tokenizer import detect_language

        with patch("workbench.tokenizer.detect", return_value="en"):
    ...
    def test_returns_detected_language_code(self):
        ...
        with patch("workbench.tokenizer.detect", return_value="fr"):
    ...
    def test_returns_en_on_lang_detect_exception(self):
        ...
        with patch("workbench.tokenizer.detect", side_effect=LangDetectException(0, "error")):
    ...
    def test_empty_text_returns_en(self):
        ...
        with patch("workbench.tokenizer.detect", side_effect=LangDetectException(0, "empty")):
            result = detect_language("")
...
class TestEfficiencyScore:
    ...
    def test_returns_float(self):
        ...
        from workbench.tokenizer import efficiency_score
...
class TestRelativeTokenizationCost:
    ...
    def test_returns_float(self):
        from workbench.tokenizer import relative_tokenization_cost
...
class TestBytePremium:
    ...
    def test_returns_float(self):
        from workbench.tokenizer import byte_premium
...
class TestContextWindowUsage:
    ...
    def test_returns_float(self):
        from workbench.tokenizer import context_window_usage
...
class TestQualityRiskLevel:
    ...
    def test_returns_string(self):
        from workbench.tokenizer import quality_risk_level
...
class TestRenderTokensHtml:
    ...
    def test_returns_string(self):
        ...
        from workbench.tokenizer import render_tokens_html
...
class TestHandleSingle:
    ...
    def test_returns_html_and_stats(self):
        ...
        from workbench.tokenizer import _handle_single
        ...
        with patch("workbench.tokenizer.get_tokenizer", return_value=mock_tok):
    ...
    def test_error_returns_empty_html_and_error_message(self):
        ...
        with patch("workbench.tokenizer.get_tokenizer", side_effect=ValueError("unknown")):
    ...
    def test_context_usage_always_shown(self):
        ...
        with patch("workbench.tokenizer.get_tokenizer", return_value=self._make_mock_tok(3)):
    ...
    def test_english_text_shows_rtc_one(self):
        ...
        with patch("workbench.tokenizer.get_tokenizer", return_value=self._make_mock_tok(3)):
            with patch("workbench.tokenizer.detect_language", return_value="en"):
                _, stats = _handle_single("gpt2", "hello world", 3, False)
    ...
    def test_non_english_with_english_text_shows_rtc(self):
        ...
        with patch("workbench.tokenizer.get_tokenizer", return_value=source_tok):
            with patch("workbench.tokenizer.detect_language", return_value="ar"):
                with patch("workbench.tokenizer.tokenize_text") as mock_tt:
                    mock_tt.side_effect = [
...
class TestHandleCompare:
    ...
    def test_returns_two_html_and_ratio_markdown(self):
        ...
        from workbench.tokenizer import _handle_compare
        ...
        with patch("workbench.tokenizer.get_tokenizer", side_effect=[mock_tok_a, mock_tok_b]):
    ...
    def test_error_returns_empty_and_error_message(self):
        ...
        with patch("workbench.tokenizer.get_tokenizer", side_effect=ValueError("bad")):
    ...
    def test_compare_with_english_text_shows_rtc(self):
        ...
        with patch("workbench.tokenizer.get_tokenizer", side_effect=[tok_a, tok_b]):
            with patch("workbench.tokenizer.tokenize_text") as mock_tt:
                mock_tt.side_effect = [
...
class TestBuildTokenizerUi:
    ...
    def test_single_status_wrapper_reports_progress_and_completion(self):
        from workbench.tokenizer import _handle_single_with_status

        with patch("workbench.tokenizer._handle_single", return_value=("<p>tokens</p>", "**Tokens:** 3")):
    ...
    def test_compare_status_wrapper_reports_progress_and_completion(self):
        from workbench.tokenizer import _handle_compare_with_status

        with patch("workbench.tokenizer._handle_compare", return_value=("<p>a</p>", "<p>b</p>", "**gpt2:** 2 tokens")):
...
class TestGetTokenizerThreadSafety:
    ...
    def test_concurrent_calls_load_tokenizer_only_once(self):
        ...
        with patch("workbench.tokenizer.AutoTokenizer.from_pretrained", side_effect=slow_from_pretrained):
            def thread_fn():
                barrier.wait()
                tok_module.get_tokenizer("gpt2")
