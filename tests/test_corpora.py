...
from workbench.corpora import fetch_strict_parallel_samples, get_corpus, list_corpora
...
class TestFetchStrictParallelSamplesResilience:
    ...
    def test_reads_local_snapshot_without_network(self, tmp_path):
        from workbench.corpora import fetch_strict_parallel_samples
        ...
        with patch("workbench.corpora.STRICT_PARALLEL_SNAPSHOT_PATH", snapshot):
            with patch("workbench.corpora._fetch_first_rows", side_effect=AssertionError("network should not be used")):
    ...
    def test_network_failure_returns_empty_not_raises(self):
        ...
        with patch("workbench.corpora.STRICT_PARALLEL_SNAPSHOT_PATH", Path("/tmp/does-not-exist.jsonl")):
            with patch("workbench.corpora._fetch_first_rows", side_effect=Exception("network error")):
    ...
    def test_partial_failure_preserves_successful_languages(self):
        ...
        with patch("workbench.corpora.STRICT_PARALLEL_SNAPSHOT_PATH", Path("/tmp/does-not-exist.jsonl")):
            with patch("workbench.corpora._fetch_first_rows", side_effect=_mock_fetch):
...
class TestStreamingExplorationSamples:
    def test_streaming_fetch_uses_remote_rows(self):
        ...
        with patch("workbench.corpora._fetch_streaming_rows", return_value={
            "fr": [
    ...
    def test_streaming_fetch_raises_clear_error_when_remote_fails(self):
        ...
        with patch("workbench.corpora._fetch_streaming_rows", side_effect=RuntimeError("stream unavailable")):
...
class TestFetchFirstRowsFailureCache:
    ...
    def setup_method(self):
        from workbench.corpora import _fetch_cache
        _fetch_cache.clear()

    def test_successful_result_is_cached(self):
        from unittest.mock import patch

        from workbench.corpora import _fetch_cache, _fetch_first_rows

        mock_resp = MagicMock()
        mock_resp.raise_for_status = MagicMock()
        mock_resp.json.return_value = {"rows": [{"row": {"text": "hello"}}]}

        with patch("workbench.corpora.requests.get", return_value=mock_resp) as mock_get:
    ...
    def test_exception_not_cached(self):
        ...
        with patch("workbench.corpora.requests.get", return_value=mock_resp):
    ...
    def test_second_call_after_failure_retries(self):
        ...
        from workbench.corpora import _fetch_first_rows
        ...
        with patch("workbench.corpora.requests.get", side_effect=[fail_resp, success_resp]):
            with pytest.raises(Exception):
    ...
    def test_separate_keys_cached_independently(self):
        ...
        with patch("workbench.corpora.requests.get", side_effect=[make_resp("a"), make_resp("b")]):
