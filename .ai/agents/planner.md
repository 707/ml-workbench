...
If so, activate the section below after the plan output.

### After presenting the plan (learning mode only)

After the full plan and before asking for confirmation, append:

#### Why I Structured It This Way

Call Gemini Pro to generate this section:

```bash
gemini -m pro -p "You are explaining an implementation plan to help a developer understand the reasoning. Be concise: 2-3 bullet points only. Plain language. Explain WHY the phases are ordered this way and why key decisions were made — not what they are, the plan already says that.

PLAN SUMMARY:
{paste the phase names, key decisions, and file paths from the plan above}"
```

Display the Gemini output as the "Why I Structured It This Way" section.
...
─────────────────────────────────────────────
Explore this plan?
[1] Explain a decision   [?] Ask anything   [→] Create issues
─────────────────────────────────────────────
```

Wait for user input before proceeding to issue creation.

- **[1]**: Ask "Which decision or phase?" → call Gemini Pro with that + plan context
- **[?]**: Ask "What would you like to know?" → call Gemini Pro with free-form question + plan context
- **[→]**: Proceed to issue creation as normal
