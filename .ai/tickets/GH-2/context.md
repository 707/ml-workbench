...
`GH-2` is **complete**. The Tokenizer Inspector is deployed and working.

### Next Work: Token Tax Calculator (GH-3 through GH-8)

Research and planning session on 2026-03-25 produced 6 new issues to evolve the Tokenizer Inspector into a multilingual LLM deployment decision tool. All ticket contexts are written and ready for TDD implementation.

**Start here:** `/tdd GH-3` (token tax metrics) — this has no dependencies and is the foundation for everything else.

**Implementation order:**
1. `/tdd GH-3` — pure metric functions (RTC, byte premium, context usage, quality risk)
2. `/tdd GH-4` — pricing data module (can run parallel with GH-3)
3. `/tdd GH-5` — Token Tax Dashboard tab (needs GH-3 + GH-4)
4. `/tdd GH-7` — enrich existing inspector (needs GH-3, parallel with GH-5)
5. `/tdd GH-6` — Traffic Analysis with CSV upload (needs GH-5)
6. `/tdd GH-8` — Plotly bubble chart (needs GH-5)

**Key artifacts:**
- Plan: `.claude/plans/gentle-growing-brook.md`
- Research: `sources.md` (8 papers on multilingual tokenization costs)
- Checkpoint: `6f85c0e` on `feature/GH-2-tokenizer-inspector`
- All ticket contexts: `.ai/tickets/GH-{3..8}/context.md`

**Platform decision:** Stay on Gradio/HF Spaces (adequate for 4-5 tabs with native charts).
